...
class Game {
    ...
    constructor () {
        ...
        this.scoreView = document.getElementById("scoreli"); // Visor puntuación
        this.livesView = document.getElementById("livesli"); // Visor de vidas
        this.youWin = false;
        this.gameOver = false;
    }
    ...
    endGame () {
        this.ended = true;
        if (this.gameOver) {
            let endGame = new Entity(this, this.width / 2, "auto", this.width / 4, this.height / 4, 0, GAME_OVER_PICTURE)
            endGame.render();
            this.gameOver = false;
        }
        else if (this.youWin) {
    ...
    update () {
        if (!this.ended) {
            this.player.update();
            if (this.opponent === undefined && this.opponentDead == false) {
                this.opponent = new Opponent(this);
            }
            if (this.opponent === undefined && this.opponentDead == true) { // Si ya está muerto el oponente, aparece el Jefe
